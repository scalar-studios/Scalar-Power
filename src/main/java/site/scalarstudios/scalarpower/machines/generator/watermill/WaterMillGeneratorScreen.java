...
public class WaterMillGeneratorScreen extends AbstractContainerScreen<WaterMillGeneratorMenu> {
    ...
    private static final int INFO_BOX_Y = 17;
    private static final int INFO_BOX_WIDTH = 74;
    private static final int INFO_BOX_HEIGHT = 52;

    public WaterMillGeneratorScreen(WaterMillGeneratorMenu menu, Inventory inventory, Component title) {
    ...
    private void drawInfo(GuiGraphicsExtractor graphics, int x, int y) {
        String generationText = menu.getSpuPerTick() + " SPU/t";
        String northSouthText = "N: " + menu.getNorthSpu() + "  S: " + menu.getSouthSpu();
        String eastWestText = "E: " + menu.getEastSpu() + "  W: " + menu.getWestSpu();
        String maxText = "Max: 32 SPU/t";

        int maxLineWidth = Math.max(this.font.width(generationText),
                Math.max(this.font.width(northSouthText),
                        Math.max(this.font.width(eastWestText), this.font.width(maxText))));
        int textX = x + INFO_BOX_X + Math.max(0, (INFO_BOX_WIDTH - maxLineWidth) / 2);

        int lineStep = this.font.lineHeight + 1;
        int totalTextHeight = lineStep * 4 - 1;
        ...
        graphics.text(this.font, northSouthText, textX, firstLineY + lineStep, ENERGY_TEXT_COLOR, false);
        graphics.text(this.font, eastWestText, textX, firstLineY + (lineStep * 2), ENERGY_TEXT_COLOR, false);
        graphics.text(this.font, maxText, textX, firstLineY + (lineStep * 3), ENERGY_TEXT_COLOR, false);
    }
